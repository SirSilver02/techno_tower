...
PriorityQueue		=	require(folder .. "priorityqueue")
Shadows				=	require(folder)
Transform			=	require(folder .. "transform")

CircleShadow		=	require(folder .. "shadowshapes.circleshadow")
PolygonShadow		=	require(folder .. "shadowshapes.polygonshadow")
...
function Body:Update()
	...
	if self.TrackBody then
		...
		if self.Transform:SetPosition( self.TrackBody:getPosition() ) or self.Transform:SetRotation( math.deg( self.TrackBody:getAngle() ) ) then
			...
			self:SetAngle(math.deg(self.TrackBody:getAngle()))  --HOTFIX Sir_Silver
